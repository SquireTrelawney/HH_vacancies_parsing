    ...
    for query_type, level, direction, query_string in zip(queries['Тип'], queries['Уровень'], queries['Направление'], queries['Ключевое слово']):
        try:
            ...
            for item in vacancies_from_response:
                for vacancy in item:
                    cursor.execute(f"SELECT * FROM vacancies_short WHERE vacancy_id={vacancy['id']} AND query_string='{query_string}'")
                    ...
                    if len(cursor.fetchall()) == -5:
                        ...
                        cursor.execute(f'INSERT INTO vacancies_short VALUES {tuple_to_insert}')
                        connection.commit()
        except Exception as E:
            print(f'!!!!! {E}')
